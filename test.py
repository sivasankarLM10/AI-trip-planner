...
Plan a trip from {BoardingPlace} to {destination} for starting date from {departure_date} to arrival date {arrival_date} with a budget of {budget} along with suggestions of the best places to stay, food and activities. Make sure to look up the most efficient route and the estimated cost of transportation and also consider {additional_information}.
(Followups)[Creative suggestions] and print each days work in a seperate tabular format.
'''.strip()
...
st.subheader('Let’s plan a magical trip! Input your budget, starting and destination place, and number of days and let Voyagical do the rest. We’ll generate a street view of the places to visit, the most efficient route, and the estimated cost of transportation. 🌍🤩')
